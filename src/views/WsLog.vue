...
<script setup lang="ts">
	...
	const logRows = ref<[{ src: string; lvl: string; msg: string; ts: number }] | []>([])
	const loggers = ref([])
	...
	wsBBClient.vSub("o", (args) => {
		console.log("LOG:", args)
		logRows.value.push(args)
		if(logRows.value.length > maxLogLines.value) logRows.value.shift()
		// if (log.value.length > maxLogLines) log.value.shift()
	})
	wsBBClient.vSub("common.log.loggers", (args) => {
